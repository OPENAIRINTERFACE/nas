...
func TestNasTypeNewPDUSessionReactivationResultErrorCause(t *testing.T) {
	a := nasType.NewPDUSessionReactivationResultErrorCause(
...
func TestNasTypePDUSessionReactivationResultErrorCauseGetSetPDUSessionIDAndCauseValue(
	t *testing.T,
) {
	a := nasType.NewPDUSessionReactivationResultErrorCause(
		nasMessage.RegistrationAcceptPDUSessionReactivationResultErrorCauseType,
	...
	for _, table := range nasTypePDUSessionIDAndCauseValueTable {
		...
		assert.Equalf(
			t,
			table.out,
			a.GetPDUSessionIDAndCauseValue(),
			"in(%v): out %v, actual %x",
			table.in,
			table.out,
			a.GetPDUSessionIDAndCauseValue(),
...
var pDUSessionReactivationResultErrorCauseInformationTable = []testPDUSessionReactivationResultErrorCauseDataTemplate{
	{
		pDUSessionReactivationResultErrorCauseTestData[0],
		pDUSessionReactivationResultErrorCauseExpectedTestData[0],
	},
}
